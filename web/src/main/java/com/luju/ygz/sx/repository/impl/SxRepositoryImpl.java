...
public class SxRepositoryImpl implements SxRepositoryI{
    ...
    public List<DcPlanInfo> select4SdList() {
        String sql = "SELECT dcId,dcNumber,dcStartTime,dcEndTime,dcType,dcTypeE,dcSource,dcDestination,dcDj,dcPath,dcIsUpdate,dcDH,jcSumHc FROM dc_show_data where  (dcXD = 'SD' OR dcXD = 'SZ' OR dcXD = 'SF') AND dcStartTime > now()  AND dcStartTime < ADDDATE(now(),interval 10800 second) order by dcStartTime";
        Object[] args = {};
        try {
    ...
    private class XxDataRowMapper implements RowMapper<DcPlanInfo> {
        public DcPlanInfo mapRow(ResultSet resultset, int i) throws SQLException {
            ...
            }else if (source != null && source.equals(ConstantFields.ZC2SOURCE)){
                dcPlanInfo.setDcSource(ConstantFields.ZC2SOURCEC);}
            else{
            ...
            if (des != null && des.equals(ConstantFields.S)) {
                ...
                dcPlanInfo.setDcDestination(des);
            }
            return dcPlanInfo;
        }
    }
