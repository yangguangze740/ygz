...
public class PrinterUtils implements Printable {
    private List<Print> listModel; //打印内容主题
    private String title; // 小票标题
    private Integer height = 100; // 打印纸高度
    private Integer marginLeft = 50; // 标题距左距离
    private Integer marginTop = 10; // 标题距上距离
    ...
    public int print(Graphics g, PageFormat pf, int page) throws PrinterException {
        ...
        for (Print m : listModel) {
            if (!m.getIsWrap()) {
                if(StringUtils.isNotBlank(m.getName())){
                    g2d.drawString(m.getName() + m.getValue(), 7, 30 + i * 24);
                ...
                g2d.drawString(m.getValue(), 7, 30 + i * 24);
            }
    ...
    public PrinterUtils(List<Print> listModel, String title, Integer height, Integer marginLeft, Integer marginTop) {
    ...
    public PrinterUtils(List<Print> listModel, String title, Integer height) {
        this.listModel = listModel;
        this.title = title;
        this.height = height;
    }

    public PrinterUtils(List<Print> listModel, String title) {
        this.listModel = listModel;
        this.title = title;
    }
